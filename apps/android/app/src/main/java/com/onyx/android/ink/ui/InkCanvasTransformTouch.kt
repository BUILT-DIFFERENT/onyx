...
private fun endPanVelocityTracking(
    ...
    if (tracker != null && fling) {
        ...
        // Optimization: avoid hypot overhead by calculating distance squared
        if ((velocityX * velocityX + velocityY * velocityY) >= (PAN_FLING_MIN_VELOCITY_PX_PER_SECOND * PAN_FLING_MIN_VELOCITY_PX_PER_SECOND)) {
...
private fun readTwoPointerTransform(event: MotionEvent): TwoPointerTransform? {
    ...
    // Optimization: calculate distance squared to avoid hypot overhead
    val dx = x1 - x0
    val dy = y1 - y0
