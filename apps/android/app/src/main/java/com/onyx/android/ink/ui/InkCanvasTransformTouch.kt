...
private fun endPanVelocityTracking(
    ...
    if (tracker != null && fling) {
        ...
        // Bolt: Opting for direct Float math instead of hypot() to avoid Double conversion overhead in this hot loop.
        if (sqrt(velocityX * velocityX + velocityY * velocityY) >= PAN_FLING_MIN_VELOCITY_PX_PER_SECOND) {
...
private fun readTwoPointerTransform(event: MotionEvent): TwoPointerTransform? {
    ...
    // Bolt: Direct math on Floats avoids the overhead of Double casts and overflow checks that hypot() does.
