...
private const val PAN_FLING_MIN_VELOCITY_PX_PER_SECOND_SQUARED = 250f * 250f
...
private fun endPanVelocityTracking(
    ...
    if (tracker != null && fling) {
        ...
        // Bolt ⚡ Optimization: Replace hypot with a squared float comparison.
        // This avoids sqrt() completely and skips standard library overhead during rapid touch gesture flings.
        val velocitySquared = (velocityX * velocityX) + (velocityY * velocityY)
        if (velocitySquared >= PAN_FLING_MIN_VELOCITY_PX_PER_SECOND_SQUARED) {
...
private fun readTwoPointerTransform(event: MotionEvent): TwoPointerTransform? {
    ...
    val dx = x1 - x0
    val dy = y1 - y0
    // Bolt ⚡ Optimization: Manually compute magnitude via float multiplication and sqrt.
    // Avoids hypot() overhead during the 120Hz gesture sampling loop.
