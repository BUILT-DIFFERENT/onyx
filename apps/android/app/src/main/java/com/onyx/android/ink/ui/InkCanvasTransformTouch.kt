...
private fun endPanVelocityTracking(
    ...
    if (tracker != null && fling) {
        ...
        // Bolt: Optimized from hypot() to squared distance calculation to avoid double conversion and overhead
        val velocitySq = velocityX * velocityX + velocityY * velocityY
        if (velocitySq >= PAN_FLING_MIN_VELOCITY_PX_PER_SECOND_SQ) {
...
private fun readTwoPointerTransform(event: MotionEvent): TwoPointerTransform? {
    ...
    // Bolt: Optimized from hypot() which performs Double conversion, underflow/overflow handling
    val dx = x1 - x0
    val dy = y1 - y0
