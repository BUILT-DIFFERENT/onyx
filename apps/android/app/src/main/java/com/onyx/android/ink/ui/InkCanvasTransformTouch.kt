...
private fun endPanVelocityTracking(
    ...
    if (tracker != null && fling) {
        ...
        // Bolt Optimization: Comparing squared limits avoids expensive Math.hypot() and Double casts in hot path.
        val velocitySq = velocityX * velocityX + velocityY * velocityY
        val thresholdSq = PAN_FLING_MIN_VELOCITY_PX_PER_SECOND * PAN_FLING_MIN_VELOCITY_PX_PER_SECOND
...
private fun readTwoPointerTransform(event: MotionEvent): TwoPointerTransform? {
    ...
    val dx = x1 - x0
    val dy = y1 - y0
    // Bolt Optimization: sqrt(dx*dx + dy*dy) avoids the Double conversion and edge-case checking overhead of hypot().
