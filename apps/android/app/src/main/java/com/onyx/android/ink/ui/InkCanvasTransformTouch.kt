...
private fun endPanVelocityTracking(
    ...
    if (tracker != null && fling) {
        ...
        // ⚡ Bolt: Use squared distance to avoid expensive hypot() calculation in gesture hot loop
...
private fun readTwoPointerTransform(event: MotionEvent): TwoPointerTransform? {
    ...
    // ⚡ Bolt: Use manual sqrt instead of hypot() to avoid overflow/underflow checking overhead
    val dx = x1 - x0
    val dy = y1 - y0
