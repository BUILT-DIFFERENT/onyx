...
private fun endPanVelocityTracking(
    ...
    if (tracker != null && fling) {
        ...
        // Using mathematically equivalent squared float math instead of hypot to avoid type cast overhead in gesture loop
        val velocityMagnitudeSq = (velocityX * velocityX) + (velocityY * velocityY)
        val minVelocitySq = PAN_FLING_MIN_VELOCITY_PX_PER_SECOND * PAN_FLING_MIN_VELOCITY_PX_PER_SECOND
        if (velocityMagnitudeSq >= minVelocitySq) {
...
private fun readTwoPointerTransform(event: MotionEvent): TwoPointerTransform? {
    ...
    // Using direct sqrt on floats instead of hypot to avoid type conversions and scaling overhead
