...
private fun endPanVelocityTracking(
    ...
    if (tracker != null && fling) {
        ...
        // ⚡ Bolt: Replace expensive hypot() with squared distance comparison
        // to avoid overflow/underflow checking overhead in hot loop
        val minSpeedSq = PAN_FLING_MIN_VELOCITY_PX_PER_SECOND * PAN_FLING_MIN_VELOCITY_PX_PER_SECOND
        if (velocityX * velocityX + velocityY * velocityY >= minSpeedSq) {
...
private fun readTwoPointerTransform(event: MotionEvent): TwoPointerTransform? {
    ...
    // ⚡ Bolt: Manually compute distance with sqrt(dx*dx + dy*dy) instead of hypot() for better performance
