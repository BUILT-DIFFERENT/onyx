...
| `image` | Scaffolded contract | Decode and keep metadata only | Same note-level unsupported hint |
| `text` | Scaffolded contract | Decode and keep metadata only | Same note-level unsupported hint |
...
1. Contract tests validate `shape` fixture parsing.
