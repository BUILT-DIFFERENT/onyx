...
- Docs path clarification: `docs/README.md`, `docs/architecture/system-overview.md`, and `docs/architecture/testing.md` now exist and are the primary repo doc entry points; use `docs/architecture/competitive-gap-backlog.md` for parity/backlog status details.
...
- Notepads hygiene clarification: `.sisyphus/notepads/README.md` is the canonical entry point for active tracker notes; historical session/final snapshots are moved under `.sisyphus/notepads/_archive/` and should not be treated as active task state.
- Canvas-overhaul status clarification: `.sisyphus/plans/milestone-canvas-rendering-overhaul.md` checkbox state can lag implementation; use `.sisyphus/notepads/milestone-canvas-rendering-overhaul/status-reconciliation-2026-03-04.md` as the current task-by-task reality map until the plan file is re-synced.
